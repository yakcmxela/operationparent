<?php get_header(); ?>
	...
	<div class="container">
		...
		<ol class="msw-resource-list flex-columns-4">
			<?php while( have_posts() ) : the_post(); ?>
				...
				<li class="msw-resource-item block">
					<?php if($resource_type == 'video_embed') : ?>
						...
						<?php endif; ?>
					<?php elseif($resource_type == 'link') : ?>
						<a target="_blank" href="<?php echo get_field('link_to') ?>">
							<div class="msw-resource-image">
								<?php if(get_field('custom_image')) : ?>
									<div class="link-image" style="background-image: url(<?php echo get_field('custom_image')['url']; ?>);"></div>
								<?php else : ?>
									<img src="<?php echo get_bloginfo('stylesheet_directory'); ?>/assets/link-icon.png">
								<?php endif; ?>
							</div>
						</a>
					<?php else : ?>
